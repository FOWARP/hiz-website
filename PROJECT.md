...
## 🤖 새 환경에서 시작할 때 — Claude가 할 일

**오누리가 "HIZ 웹사이트 이어서 하자"고 하면 Claude가 직접 아래를 수행한다. 오누리에게 명령어를 알려주거나 직접 입력하게 시키지 않는다.**

1. `~/hiz-website` 폴더가 있는지 확인
2. 없으면 Claude가 직접 터미널에서 `git clone https://github.com/FOWARP/hiz-website ~/hiz-website` 실행
3. 이 PROJECT.md 읽어서 현재 상태 파악
4. 오누리에게 현재 진행 상황을 한국어로 요약해서 말해주고, 뭐 이어서 할지 물어보기
